  ...
  constructor(chatDetails, region, customNetworkStatus) {
    ...
    try {
      if (chatDetails) {
        ...
        console.log(
          'startChatResult:',
          JSON.stringify(startChatDetails, null, 2),
        );
...
class ChatSession {
  ...
  sendReadReceipt(messageId, options) {
    this.logger &&
      this.logger.info(
        'Calling SendEvent API for ReadReceipt',
        messageId,
        options,
      );;
  ...
  sendDeliveredReceipt(messageId, options) {
    this.logger &&
      this.logger.info(
        'Calling SendEvent API for DeliveredReceipt',
        messageId,
        options,
      );
  ...
  addOutgoingMessage(data) {
    ...
    this.logger &&
      this.logger.info(`Adding outgoing message. ContactId: ${this.contactId}`);
  ...
  addOutgoingAttachment(attachment) {
    ...
    this.logger &&
      this.logger.info(`Sending File. ContactId: ${this.contactId}.`);
    ...
    return this.client
      ...
      .catch(error => {
        ...
        if (error.type !== AttachmentErrorType.ValidationException) {
          if (
            error.type === AttachmentErrorType.ServiceQuotaExceededException
          ) {
    ...
    return this.client
      ...
      .then(response => {
        ...
        const transcriptItems = incomingDataList.map(data => {
          const transcriptItem = modelUtils.createItemFromIncoming(
            data,
            this.thisParticipant,
          );;
  ...
  _handleMessageReceipt(messageReceiptType, dataInput) {
    ...
    const oldItemInTranscript =
      this._findItemInTranscriptUsingMessageId(messageId);

    if (oldItemInTranscript === -1) {
      this.logger &&
        this.logger.debug(
          `Message with messageId:${messageId} not found in transcript`,
        );;
      return;
    }
    const {sentTime} = oldItemInTranscript.transportDetails;;
    this._handleMessageReceiptLatencyMetric(
      messageReceiptType,
      dataInput,
      sentTime,
    );
  ...
  _findReceipt(receipts, participantId) {
    return receipts.find(
      receipt => receipt.RecipientParticipantId !== participantId,
    );
  }
  ...
  _addItemsToTranscript(items) {
    ...
    const lastReadMessageIdx = this._findLastMessageReceiptInTranscript(
      'read',
      newTranscript,
    );
    const lastDeliveredMessageIdx = this._findLastMessageReceiptInTranscript(
      'delivered',
      newTranscript,
    );

    const lastIncomingMessageIdx = this._findLastMessageInTranscript(
      Direction.Incoming,
      newTranscript,
    );
    const lastOutgoingMessageIdx = this._findLastMessageInTranscript(
      Direction.Outgoing,
      newTranscript,
    );
    ...
    if (
      lastDeliveredMessageIdx !== -1 &&
      lastReadMessageIdx < lastDeliveredMessageIdx
    ) {
  ...
  _findItemInTranscriptUsingMessageId(messageId) {
    const index = this.transcript.findIndex(
      transcript => transcript.id === messageId,
    );
    if (index !== -1) {
  ...
  _handleTypingEvent(dataInput) {
    ...
    const incomingTypingParticipant = modelUtils.createTypingParticipant(
      data,
